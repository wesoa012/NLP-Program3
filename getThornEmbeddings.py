...
# sense1_sentences.append("Her annoying voice was a thorn in my side all day long.")
# sense1_sentences.append("Always eating out had become a thorn in her daily cooking routine.")
# sense1_sentences.append("Another thorn is that commercial success is seen as incompatible with creative integrity.")

# sense2_sentences.append("A thorn embedded itself into the palm of my hand.")
# sense2_sentences.append("When they trample Mama's flower beds, she replaces the flowers with thorn bushes to trip them up so that they can not get too close to the house.")
# sense2_sentences.append("And if he is pierced with a thorn, let him not find anyone to take it out for him.")

with open('./Data/txts/Thorn.txt', 'r') as file:
    for line in file.readlines():
        if 'thorn' not in line:
            continue
        else:
            if line[0] == '1':
...
with torch.no_grad():
    ...
    sense1_outputs = thorn_BERT(sense1_tokens_tensor, sense1_segments_tensors)
    ...
    sense2_outputs = thorn_BERT(sense2_tokens_tensor, sense2_segments_tensors)
...
for i in range(len(sense1_hidden_states[0])):
    for j in range(len(sense1_encodings[i])):
        if sense1_encodings[i].tokens[j] == 'thorn':
            # print("found thorn")
...
for i in range(len(sense2_hidden_states[0])):
    for j in range(len(sense2_encodings[i])):
        if sense2_encodings[i].tokens[j] == 'thorn':
            # print("found thorn")
...
if(os.path.exists('./Data/embeddings/thorn_cos_sims_sense1_only.csv')):
    print("skipping creation of cos_sims for sense 1")
    do_sense1_only = False
if(os.path.exists('./Data/embeddings/thorn_cos_sims_sense2_only.csv')):
    print("skipping creation of cos_sims for sense 2")
    do_sense2_only = False
if(os.path.exists('./Data/embeddings/thorn_cos_sims_both_senses.csv')):
...
if(do_sense1_only):
    ...
    cos_sims_sense1_only_df.to_csv('./Data/embeddings/thorn_cos_sims_sense1_only.csv')
else:
    print("Reading dataframe/csv for cosine similarities for sense 1 only")
    cos_sims_sense1_only_df = pd.read_csv('./Data/embeddings/thorn_cos_sims_sense1_only.csv')
...
if(do_sense2_only):
    ...
    cos_sims_sense2_only_df.to_csv('./Data/embeddings/thorn_cos_sims_sense2_only.csv')
else:
    print("Reading dataframe/csv for cosine similarities for sense 2 only")
    cos_sims_sense2_only_df = pd.read_csv('./Data/embeddings/thorn_cos_sims_sense2_only.csv')
...
if(do_both_senses):
    ...
    cos_sims_both_df.to_csv('./Data/embeddings/thorn_cos_sims_both_senses.csv')
else:
    print("Reading dataframe/csv for cosine similarities for both senses")
    cos_sims_both_df = pd.read_csv('./Data/embeddings/thorn_cos_sims_both_senses.csv')

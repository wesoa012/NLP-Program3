...
# sense1_sentences.append("Rubbish was scattered all over the place.")
# sense2_sentences.append("Her argument was rubbish.")

with open('./Data/txts/Rubbish.txt', 'r') as file:
    for line in file.readlines():
        if 'rubbish' not in line:
            continue
        else:
            if line[0] == '1':
...
print(f"lengths = {len(sense1_sentences)}, {len(sense2_sentences)}")
...
with torch.no_grad():
    ...
    sense1_outputs = rubbish_BERT(sense1_tokens_tensor, sense1_segments_tensors)
    ...
    sense2_outputs = rubbish_BERT(sense2_tokens_tensor, sense2_segments_tensors)
...
for i in range(len(sense1_hidden_states[0])):
    for j in range(len(sense1_encodings[i])):
        if sense1_encodings[i].tokens[j] == 'rubbish':
...
for i in range(len(sense2_hidden_states[0])):
    for j in range(len(sense2_encodings[i])):
        if sense2_encodings[i].tokens[j] == 'rubbish':
...
plt.savefig('./Graphs/Sentence_Embeddings_Rubbish.png')
...
plt.savefig('./Graphs/Sentence_Embeddings_Flipped_Rubbish.png')

cos_sims_sense1_only, cos_sims_sense2_only, cos_sims_both = [],[],[]
do_sense1_only, do_sense2_only, do_both_senses = True,True,True
if(os.path.exists('./Data/embeddings/rubbish_cos_sims_sense1_only.csv')):
    print("skipping creation of cos_sims for sense 1")
    do_sense1_only = False
if(os.path.exists('./Data/embeddings/rubbish_cos_sims_sense2_only.csv')):
    print("skipping creation of cos_sims for sense 2")
    do_sense2_only = False
if(os.path.exists('./Data/embeddings/rubbish_cos_sims_both_senses.csv')):
...
if(do_sense1_only):
    ...
    cos_sims_sense1_only_df.to_csv('./Data/embeddings/rubbish_cos_sims_sense1_only.csv')
else:
    print("Reading dataframe/csv for cosine similarities for sense 1 only")
    cos_sims_sense1_only_df = pd.read_csv('./Data/embeddings/rubbish_cos_sims_sense1_only.csv')
...
if(do_sense2_only):
    ...
    cos_sims_sense2_only_df.to_csv('./Data/embeddings/rubbish_cos_sims_sense2_only.csv')
else:
    print("Reading dataframe/csv for cosine similarities for sense 2 only")
    cos_sims_sense2_only_df = pd.read_csv('./Data/embeddings/rubbish_cos_sims_sense2_only.csv')
...
if(do_both_senses):
    ...
    cos_sims_both_df.to_csv('./Data/embeddings/rubbish_cos_sims_both_senses.csv')
else:
    print("Reading dataframe/csv for cosine similarities for both senses")
    cos_sims_both_df = pd.read_csv('./Data/embeddings/rubbish_cos_sims_both_senses.csv')
...
plt.savefig('./Graphs/Rubbish_Sense1_Only_SimVals.png')
...
ax.set_xlabel('Similarity Value',fontsize=15)
ax.set_ylabel('Density',fontsize=15)
plt.savefig('./Graphs/Rubbish_Sense2_Only_SimVals.png')
...
ax.set_xlabel('Similarity Value',fontsize=15)
ax.set_ylabel('Density',fontsize=15)
plt.savefig('./Graphs/Rubbish_Both_Senses_SimVals.png')
